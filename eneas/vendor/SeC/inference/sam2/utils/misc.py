...
def get_connected_components(mask):
    ...
    """
    # Try to use the compiled CUDA extension first (fastest)
    try:
        from sam2 import _C

        return _C.get_connected_componnets(mask.to(torch.uint8).contiguous())
    except (ImportError, ModuleNotFoundError):
        # Fallback to OpenCV implementation (pure Python, works on CPU/CUDA)
        import cv2
        ...
        # Initialize outputs
        labels_out = torch.zeros_like(mask, dtype=torch.int32)
        counts_out = torch.zeros_like(mask, dtype=torch.int32)
        ...
        for i in range(N):
            ...
            # Use OpenCV connectedComponentsWithStats (8-connectivity)
            ...
            # Create area map
            areas = np.zeros_like(labels, dtype=np.int32)
            for label_id in range(num_labels):
                area = stats[label_id, cv2.CC_STAT_AREA]
                areas[labels == label_id] = area

            # Convert back to tensors
            labels_out[i, 0] = torch.from_numpy(labels.astype(np.int32))
            counts_out[i, 0] = torch.from_numpy(areas.astype(np.int32))

        # Move back to original device
        labels_out = labels_out.to(device)
        counts_out = counts_out.to(device)

        return labels_out, counts_out
...
def load_video_frames_from_jpg_images(
    ...
    # Patched by eneas: extract numbers from filename for sorting
    import re
