...
def build_sam2_video_predictor(
    ...
    **kwargs,
    ...
    hydra_overrides = [
        "++model._target_=eneas.vendor.SeC.inference.sam2_video_predictor.SAM2VideoPredictor",
    ]
    ...
    hydra_overrides_extra.append(f"model.num_maskmem={num_maskmem}")

    # Patched: Filter out grounding_encoder_config from overrides
    # This parameter is NOT accepted by SAM2Base.__init__()
    hydra_overrides_extra = [
        override for override in hydra_overrides_extra if "grounding_encoder_config" not in override
    ]
...
class SAM2VideoPredictor(_SAM2VideoPredictor):
    def init_state(self, video_path, **kwargs):
        ...
        # Patched by eneas: Use alphabetical sorting instead of numeric
        # This respects the user's frame ordering and works with any filename format
        frame_names = [
            p
            for p in os.listdir(video_path)
            if os.path.splitext(p)[-1] in [".jpg", ".jpeg", ".JPG", ".JPEG", ".png", ".PNG"]
        ]
        frame_names.sort()  # Simple alphabetical sort - respects user's naming
        inference_state["video_paths"] = [
            os.path.join(video_path, frame_name) for frame_name in frame_names
        ]
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            valid_indices = [
                ...
                if (
                    output_dict["non_cond_frame_outputs"][i]["ious"].max().item() > iou_thre
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                for t_diff in range(1, max_obj_ptrs_in_encoder):
                    ...
                    out = output_dict["non_cond_frame_outputs"].get(
                        valid_indices[-t_diff],
                        unselected_cond_outputs.get(valid_indices[-t_diff], None),
                    )
                    if out is not None:
        ...
        if mask_inputs is not None and self.use_mask_input_as_output_without_sam:
            ...
            if language_embd is not None:
                ...
                _language_embd_pos = _language_embd.new_zeros(
                    _language_embd.size(0), 1, self.mem_dim
                )
                ...
                pix_feat_with_language = pix_feat_with_language.permute(1, 2, 0).view(
                    1, 256, 64, 64
                )
